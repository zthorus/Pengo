        ...
        for (i = 1 ; i <= 16 ; i++)
        {
          ...
          for (j = 0 ; j < NB_SNOBEE ; j++)
          {
            switch(snb_state[j])
            {
              ...
              case CRUSHED:
                if (i == 1) PutSprite(xim,lut,sprite_mem,22,snb_x[j]*16,snb_y[j]*16);
                if (i == 2) EraseSprite(xim,lut,snb_x[j]*16,snb_y[j]*16);
                break;

              case REBIRTH:
                PutSprite(xim,lut,sprite_mem,24,snb_x[j]*16,snb_y[j]*16);
                break;
...
void DisplayBorder(XImage *xim,unsigned long *lut,int pmx,int pmy,int color)
{
  ...
  { 
    ...
    {
      Dot(xim,lut,i,j,color);
      Dot(xim,lut,pmx-W_BRDR+i,j,color);
...
void PutSprite(XImage *xim,unsigned long *lut,unsigned char *sprite_mem, int sprite_num, int x, int y)
{
  ...
  for (j = 0 ; j < 16 ; j++)
  {
    for (i = 0 ; i < 16 ; i++)
    {
      Dot(xim,lut,x+i-W_BRDR,y+j-W_BRDR,sprite_mem[a]);
...
void EraseSprite(XImage *xim,unsigned long *lut,int x,int y)
{
  ...
  for (j = 0 ; j < 16 ; j++)
  {
    for (i = 0 ; i < 16 ; i++)
    {
      Dot(xim,lut,x+i-W_BRDR,y+j-W_BRDR,0);
